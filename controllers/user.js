...
function defineRoutes(app) {
	...
	app.get('/users/:page?', app.pol.perm.readStrucPermOn_users, app.pol.pag.paginationInit, function(req, res, next){
		...
		if ( req.params.page == undefined || !isNaN(req.params.page) )
		{
			...
			app.pol.user.count(
				...
				function(err, total) {
						...
						app.pol.user.find(
							...
							function(err, users) {
								res.render('users/list', { 
									...
									elements: users,
									total: total,
									pagination: app.pol.pag.paginationDo(req, total, '/users/')
								});	
							}
						);	
					}
					...
					{
						app.pol.utils.errorPage(res, err, "GET: user list: failed query on db");
					}	
				}
			);
		}
	...
	//GET: user detail 
	app.get('/users/:id', app.pol.perm.readStrucPermOn_users, function(req, res, next){
		app.pol.routes.routeInit(req);
			...
			function(err, user) {
				...
				if ( !err )
				{
					...
					else
					{
						...
						//app.pol.utils.errorPage(res, err, "GET: user detail: user not found");
						res.redirect('/');
					}
				}
				else
				{
					app.pol.utils.errorPage(res, err, "GET: user detail: query error");
				}
			}
		);	
	});
	...
	app.get('/users/edit/new', app.pol.perm.readStrucPermOn_users, app.pol.perm.needStrucPermCreate, function(req, res, next){
		app.pol.routes.routeInit(req);
	...
	app.post('/users/edit', app.pol.perm.readStrucPermOn_users, app.pol.perm.needStrucPermCreate, function(req, res, next){
		app.pol.routes.routeInit(req);
		...
		app.pol.user.findOne(
			...
			function(err, user) {
				...
				{
					//email già usata
					app.pol.utils.errorPage(res, err, "already exists user with email: "+req.body.email);
				}
				else
				{
					//email libera
					//creo nuovo user
					var my_user = new app.pol.user();
					//popolo il mio user con quanto mi arriva dal form
					app.pol.utils.populateModel(my_user, req.body);
					...
					my_user.password = app.pol.sess.hashPw(req, my_user.password);
					...
					my_user.save(function (err) {
						if (!err) 
						{
							...
							app.pol.sess.setSignedIn(req, my_user.id);
							//e rimando nel form
							res.redirect('/users/edit/'+my_user.id+'/success');
						}
						else
						{
							app.pol.utils.errorPage(res, err, "POST: user form: saving user");
						}
					});
				}
			}
	...
	app.get('/users/edit/:id/:msg?', app.pol.perm.readStrucPermOn_users, app.pol.perm.needStrucPermModifyMyself, function(req, res, next){
		app.pol.routes.routeInit(req);
		...
		app.pol.user.findOne(
			...
			function(err, user) {
				...
				{
					app.pol.utils.errorPage(res, err, "GET: user form (modify): failed query on db");
				}	
	...
	app.post('/users/edit/:id', app.pol.perm.readStrucPermOn_users, app.pol.perm.needStrucPermModifyMyself, function(req, res, next){
		app.pol.routes.routeInit(req);
		...
		app.pol.user.findOne(
			...
			function(err, user) {
					...
					app.pol.user.findOne(
						...
						function(err, userSameMail) {
							...
							else
							{
								...
								//popolo il mio user con quanto mi arriva dal form
								app.pol.utils.populateModel(user, req.body);
								...
								{
									//encripto la pw
									user.password = app.pol.sess.hashPw(req, user.new_password);
								}
				...
				{
					app.pol.utils.errorPage(res, err, "POST: user form (modify): user not found on db");
				}
			}
		);
	});
	...
	//GET: user delete
	app.get('/users/delete/:id', app.pol.perm.readStrucPermOn_users, app.pol.perm.needStrucPermModify, function(req, res, next){
		app.pol.routes.routeInit(req);
		//cancello l'utente
		app.pol.user.remove(
			{ '_id': req.params.id },
			function(err, user) {
				if ( err ) app.pol.utils.errorPage(res, err, 'GET: user delete: failed query on db');
			}
		);
