...
function paginationInit(req, res, next) {
	...
	//skip
	req.session.skip = req.app.pol.config.elementsPerPage * ( req.session.pageNum - 1 );
	//limit
	req.session.limit = req.app.pol.config.elementsPerPage;
...
function paginationDo(req, total, url) {
	//actual range
	var recordsPerPage = req.app.pol.config.elementsPerPage;
