...
function checkValidUser(req, closure)
{
	...
	if ( req.app.pol.config.superadminEmail == email && req.app.pol.config.superadminPw == password )
	{
		...
		req.app.pol.user.findOne(
			...
			function(err, user) {
				...
				else
				{
					...
					errorPage(res, err, 'polenta.checkValidUser: error in query retrieving users');
				}
			}
		);		
	}
...
{
	if (password) {
		return req.app.pol.crypto.createHash('sha1').update(password).digest('hex');
	} else {
		return '';
	}
}
